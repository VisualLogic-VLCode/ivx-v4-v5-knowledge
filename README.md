...
npm run release:prepare -- \
  --version 0.1.3 \
  --previous-manifest ./release-out/knowledge-0.1.2/knowledge-stable.json
npm run release:publish -- \
  --plan ./release-out/knowledge-0.1.3/github-release-plan.json \
...
For a compatibility-only patch that does not change books, Cards, provenance, or indexes, do not run `sync:source`. Update the package/runtime version and the identical compatibility range in `runtime/manifest.json` and `config/public-export-allowlist.json`, keep the content digest and tracked content hashes unchanged, run `npm run check`, then use the same commit/push/prepare/publish sequence.
